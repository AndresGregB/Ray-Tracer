...
{
	Vec3 T;
	double cosbeta = 0, cosalfa = 0;

	//Cosenos de angulo normal y refractado
	cosalfa = N * (-V);
	cosbeta = sqrt(1 - 1/eta * (1 - pow(cosalfa,2)));

	//Direccion del Vector refractado
	T = (V/eta) + N * ((1/eta) * cosalfa - cosbeta);
	return T;
}
